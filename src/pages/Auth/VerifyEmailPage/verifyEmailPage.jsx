...
export default function VerifyEmailDemo() {
  ...
  const [currentState, setCurrentState] = useState("loading"); // loading, success, error, noToken
  ...
  useEffect(() => {
    ...
    const itv = setInterval(
      () => setCountdown((v) => (v <= 1 ? (clearInterval(itv), 0) : v - 1)),
      1000
    );
  ...
  const StateSelector = () => (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-white rounded-full shadow-lg p-2 flex gap-2 ring-1 ring-black/5">
      {[
        ["loading", "Loading"],
        ["success", "Success"],
        ["error", "Error"],
        ...
        <button
          ...
          onClick={() => {
            setCurrentState(key);
            if (key === "success") setCountdown(3);
          }}
          className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
            currentState === key
              ? "text-white bg-gradient-to-r from-pink-500 to-indigo-600 shadow"
        ...
        <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 flex items-center justify-center p-6">
          ...
          >
            <div className="bg-white rounded-3xl shadow-2xl p-8 relative overflow-hidden ring-1 ring-black/5">
              ...
              <button className="block w-full bg-gradient-to-r from-pink-500 to-indigo-600 hover:from-pink-600 hover:to-indigo-700 text-white font-semibold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 text-center">
                Về trang chủ
              </button>
            </div>
          </div>
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50 flex items-center justify-center p-6">
        ...
        >
          <div className="bg-white rounded-3xl shadow-2xl p-9 relative overflow-hidden ring-1 ring-black/5">
            {/* Loading (đã đổi sang HourglassSpinner) */}
            ...
            {currentState === "error" && (
              <>
                ...
                <h1 className="text-3xl font-bold text-center bg-gradient-to-r from-rose-600 to-red-600 bg-clip-text text-transparent mb-4">
                  Xác thực thất bại
                </h1>
                <p className="text-gray-600 text-center leading-relaxed mb-6">
      ...
      {/* keyframes cho shimmer (Tailwind arbitrary animate không có sẵn) */}
      <style>{`
        @keyframes shimmer {
